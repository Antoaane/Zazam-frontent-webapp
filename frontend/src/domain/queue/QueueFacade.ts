...
class QueueFacade {
  ...
  addTrackToQueue(track: UserTrack): void {
    this.initialize()
    const store = this.getStore()
    if (store.isReadOnly) {
      console.log('[Queue] Add track ignored (read-only)')
      return
    }
    store.appendTracks([toQueueTrack(track)])
  }
  ...
  async addPlaylistToQueue(playlistId: string): Promise<void> {
    ...
    const store = this.getStore()
    if (store.isReadOnly) {
      console.log('[Queue] Add playlist ignored (read-only)')
      return
    }

    const tracks = await userLibraryFacade.loadAllPlaylistTracks(playlistId)
    if (tracks.length === 0) {
      console.log('[Queue] Playlist has no tracks', { playlistId })
      return
    }
    store.appendTracks(tracks.map(toQueueTrack))
  }
