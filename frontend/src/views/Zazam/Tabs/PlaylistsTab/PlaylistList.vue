...
const handleSwipe = (payload: { action: 'add-to-queue'; payload?: unknown }) => {
  if (payload.action !== 'add-to-queue') {
    return
  }
  const playlistId = typeof payload.payload === 'string' ? payload.payload : ''
  if (!playlistId) {
    return
  }
  void queueFacade.addPlaylistToQueue(playlistId)
}
</script>
