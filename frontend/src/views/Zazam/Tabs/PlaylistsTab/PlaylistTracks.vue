...
const handleSwipe = (payload: { action: 'add-to-queue'; payload?: unknown }) => {
  if (payload.action !== 'add-to-queue' || !payload.payload) {
    return
  }
  queueFacade.addTrackToQueue(payload.payload as UserTrack)
}
</script>

<template>
  <div class="playlist-tracks-tab">
    <div class="px-4 py-3 flex justify-between items-center gap-3 border-b border-primary/50">
    ...
    <div ref="listRef" class="list px-2 pt-2 flex-1 flex flex-col gap-2 rounded-t-xl">
      <Item
